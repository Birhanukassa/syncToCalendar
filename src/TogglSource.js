 ...
 class TogglSource {
     constructor(config, calendarManager, errorHandler, monitor) {
         ...
         if (this.config.API_TOKEN) {
             this.apiClient = new ApiClient(
                 ...
                 {
                     'Authorization': 'Basic ' + Utilities.base64Encode(this.config.API_TOKEN + ':api_token')
                 },
         ...
         if (this.config.FILTER_BY_TAGS && this.config.FILTER_BY_TAGS.length > 0) {
             filtered = filtered.filter(entry =>
                 entry.tags && entry.tags.some(tag =>
                     this.config.FILTER_BY_TAGS.includes(tag)
                 )
             );
         }
     ...
     processEntry(entry, projects) {
         ...
         if (targetCalendar && targetCalendar.getId() !== this.calendarManager.getCalendar().getId()) {
             let altManager = this.alternateCalendars.get(targetCalendar.getId());
             if (!altManager) {
                 altManager = new CalendarManager(targetCalendar.getId(), this.errorHandler, this.globalConfig);
         ...
         if (project && this.config.PROJECT_CALENDAR_MAP) {
             ...
             if (calendarId) {
                 try {
                     ...
                     console.warn(`Could not access calendar ${calendarId} for project ${project.name}`);
                 }
             }
         }
