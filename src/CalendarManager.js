 ...
 class CalendarManager {
     ...
     flushOperations() {
         ...
         operations.forEach((op, index) => {
             try {
                 ...
                 this.errorHandler.recordError(error, `Batch operation ${index + 1}/${operations.length}`);
             }
         });
     }
     ...
     syncEvent(eventData) {
         ...
         try {
             ...
             if (existingEvent) {
                 if (this.eventNeedsUpdate(existingEvent, title, start, end, fullDescription, isAllDay)) {
                     this.queueOperation(() => {
                 ...
                 this.queueOperation(() => {
                     const newEvent = isAllDay
                         ? this.calendar.createAllDayEvent(title, start, { description: fullDescription })
                         : this.calendar.createEvent(title, start, end, { description: fullDescription });
     ...
     eventNeedsUpdate(event, newTitle, newStart, newEnd, newDescription, isAllDay) {
         try {
             ...
             if (isAllDay && event.isAllDayEvent()) {
                 ...
                 return event.getStartTime().getTime() !== newStart.getTime() ||
