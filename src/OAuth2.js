(function (host, expose) {
    ...
    function getRedirectUri(optScriptId) {
        ...
        return (
            'https://script.google.com/macros/d/' + encodeURIComponent(scriptId) + '/usercallback'
        );
    }
    ...
    Service_.prototype.getAuthorizationUrl = function (optAdditionalParameters) {
        ...
        var stateTokenBuilder = eval('Script' + 'App')
            .newStateToken()
        ...
        if (this.codeVerifier_) {
            params['code_challenge'] = encodeChallenge_(
                this.codeChallengeMethod_,
                this.codeVerifier_
            );
    ...
    Service_.prototype.hasAccess = function () {
        ...
        var canGetToken = (token && this.canRefresh_(token)) || this.privateKey_ || this.grantType_;
    ...
    Service_.prototype.getTokenFromResponse_ = function (response) {
        ...
        if (resCode < 200 || resCode >= 300 || token.error) {
            var reason = [token.error, token.message, token.error_description, token.error_uri]
                .filter(Boolean)
                .map(function (part) {
    ...
    Service_.prototype.isExpired_ = function (token) {
        ...
        if (token.id_token) {
            ...
            if (payload.exp && payload.exp - now < Service_.EXPIRATION_BUFFER_SECONDS_) {
                expired = true;
            }
        }
    ...
    Service_.prototype.canRefresh_ = function (token) {
        ...
        if (token.refreshTokenExpiresAt === undefined) {
            ...
            var now = getTimeInSeconds_(new Date());
            return token.refreshTokenExpiresAt - now > Service_.EXPIRATION_BUFFER_SECONDS_;
        }
    };
    ...
    Service_.prototype.exchangeGrant_ = function () {
        ...
        if (
            ...
            this.tokenHeaders_.authorization =
                'Basic ' + Utilities.base64Encode(this.clientId_ + ':' + this.clientSecret_);
        }
    ...
    Storage_.prototype.getValue = function (key, optSkipMemoryCheck) {
        ...
        // Check properties.
        if (this.properties_ && (jsonValue = this.properties_.getProperty(prefixedKey))) {
            if (this.cache_) {
                this.cache_.put(prefixedKey, jsonValue, Storage_.CACHE_EXPIRATION_TIME_SECONDS);
            }
            value = JSON.parse(jsonValue);
            this.memory_[prefixedKey] = value;
            return value;
        ...
        if (this.cache_) {
            this.cache_.put(
                prefixedKey,
                JSON.stringify(Storage_.CACHE_NULL_VALUE),
    ...
    function encodeJwt_(payload, key) {
        ...
        var signatureBytes = Utilities.computeRsaSha256Signature(toSign, key);
